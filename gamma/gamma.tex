...
\begin{proof}
    ...
    $$\left|f_{n+p}-f_n\right|=\left|\displaystyle\sum_{n\leqslant k < n+p}(f_{k+1}-f_k)\right|=\displaystyle\sum_{n\leqslant k < n+p}(f_{k+1}-f_k)$$
    Donc $\left|f_{n+p}-f_n\right|$ est croissante. Soit $A$ une partie majorée de $D$, notons $a=\sup A$. \\
    Soit $\epsilon>0$. Il existe $N\geqslant 0$ tel que, pour $n\geqslant N$ et $p\geqslant 0$, on a : $\left|f_{n+p}(a)-f_n(a)\right|\leqslant\epsilon$
    puisque $(f_n(a))$ converge donc vérifie le critère de Cauchy. Or $\left|f_{n+p}-f_n\right|$ est croissante donc $\left|f_{n+p}(a)-f_n(a)\right|\geqslant\underset{x\in A}{\sup}\left|f_{n+p}(x)-f_n(x)\right|$.
    Donc $\underset{x\in A}{\sup}\left|f_{n+p}(x)-f_n(x)\right|\leqslant\epsilon$. \\
