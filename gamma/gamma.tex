...
\begin{enumerate}
    ...
    \end{align*}
    On constate que $(\ln f_n)$ converge simplement, donc $(f_n)$ converge simplement vers une fonction $f:\R_+\longmapsto\R_+^*$.
    ...
    Comme $\dfrac{1}{(x+n)^2}\underset{n\rightarrow +\infty}{=}O\left(\dfrac{1}{n^2}\right)$,
...
\end{thm}
\begin{proof}
    Commençons par montrer que $f$ est KH-intégrable sur $[a,b]$. Soit $\epsilon>0$. Par converge uniforme, il existe un rang $N\geqslant 0$ tel que, avec $\epsilon'>0$ défini plus tard
    : $$\forall x\in[a,b],\ \abs{f_N(x)-f(x)}\leqslant\epsilon'\text{ \ \ \ \ \ \ \ \ soit : \ \ \ \ \ \ \ \ }f_N(x)-\epsilon'\leqslant f(x)\leqslant f_N(x)+\epsilon'$$
    Or : $$\int_a^b(f_n+\epsilon')-\int_a^b(f_n-\epsilon')=\int_a^b(2\epsilon')=2(b-a)\epsilon'=\epsilon\text{\ \ \ \ \ \ \ avec }\epsilon'=\dfrac{\epsilon}{2(b-a)}$$
